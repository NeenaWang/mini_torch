    ...
    def train(self, data, learning_rate, max_epochs=500, log_fn=default_log_fn):
        ...
        for epoch in range(max_epochs):
            ...
            if epoch % 10 == 0 or epoch == max_epochs:
                ...
                if epoch > 0:
                    print(f"Average time per epoch {total_epoch_time/epoch} (for {epoch} epochs)")
                if correct == y2.shape[0]:
                    num_epochs = epoch
                    break
        print(f"Average time is {total_epoch_time} (for {num_epochs} epochs)")
        print(f"Average time per epoch {total_epoch_time/num_epochs} (for {num_epochs} epochs)")
