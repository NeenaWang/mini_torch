...
class CNNSentimentKim(minitorch.Module):
    ...
    def forward(self, embeddings):
        ...
        out_max = minitorch.nn.max(out1, 2) + minitorch.nn.max(out2, 2) + minitorch.nn.max(out3, 2)
        out_max = out_max.view(embeddings.shape[0], self.feature_map_size)
        out = self.linear.forward(out_max)
        if self.training:
